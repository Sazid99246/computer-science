...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    if inbetween.call(x, seg.x1, seg.x2) && inbetween.call(y, seg.y1, seg.y2)
      self
    else
      NoPoints.new
    end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(x1, x2)
      # Vertical segments: y2 is the start (bottom), y1 is the end (top)
      seg1, seg2 = y2 < seg.y2 ? [self, seg] : [seg, self]
      
      if real_close(seg1.y1, seg2.y2)
        Point.new(seg1.x1, seg1.y1)
      elsif seg1.y1 < seg2.y2
        NoPoints.new
      elsif seg1.y1 > seg2.y1
        LineSegment.new(seg2.x1, seg2.y1, seg2.x2, seg2.y2)
      else
        LineSegment.new(seg1.x1, seg1.y1, seg2.x2, seg2.y2)
      end
    else
      # Non-vertical segments: x2 is the start (left), x1 is the end (right)
      seg1, seg2 = x2 < seg.x2 ? [self, seg] : [seg, self]
      
      if real_close(seg1.x1, seg2.x2)
        Point.new(seg1.x1, seg1.y1)
      elsif seg1.x1 < seg2.x2
        NoPoints.new
      elsif seg1.x1 > seg2.x2
        LineSegment.new(seg2.x1, seg2.y1, seg2.x2, seg2.y2)
      else
