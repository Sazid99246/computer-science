...
/*
         ...
         |                     |                      |           
  +--------------+    +-----------------+  +-----------------+
  | TStop        |    | CommStation     |  | BusStop         |
  +--------------+    +-----------------+  +-----------------+
  | String name  |    | String name     |  | String name     |
  | String line  |    | String line     |  | String line     |
  | double price |    | boolean express |  | boolean express |
  +--------------+    +-----------------+  | double fare     |
                                           +-----------------+
 */
...
class BusStop implements IStation {
  ...
  BusStop(String name, String line,
          boolean express, double fare) {
    this.name = name;
    this.line = line;
    this.express = express;
    this.fare = fare;
...
class ExamplesIStation{
  ...
  IStation dudley =
      new BusStop("Dudley", "Silver Line", true, 2.00);
  IStation harvardBus =
      new BusStop("Harvard Busway", "86", false, 1.70);
}