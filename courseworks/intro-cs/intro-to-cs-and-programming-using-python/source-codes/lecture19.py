...
class Rabbit(Animal):
    ...
    def __eq__(self, oth):
        ...
        parents_same = (
            self.p1.rid == oth.p1.rid and self.p2.rid == oth.p2.rid)
...
class Employee(Person):
    ...
    def past_salary_freq(self):
        ...
        for salary in self.salaries:
            if salary in salary_freq.keys():
                salary_freq[salary] += 1
            else:
                salary_freq[salary] = 1
        return salary_freq
# # For example:
...
def counts(L):
    ...
    for i in L:
        if type(i) == Person:
            ...
            num_of_employess += 1
        if i.get_name() not in unique_names:
            unique_names.append(i.get_name())

    return (num_of_persons, num_of_employess, len(unique_names))
