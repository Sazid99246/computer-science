...
def put_in_functions_a():
    try:
        ...
        FUNCTION_HEADER = 'def %s(yearly_salary, portion_saved, cost_of_dream_home):' % (
            FUNCTION_NAME)
        ...
        START_INDEX = [line.startswith(START_LINE)
                       for line in lines].index(True)
...
def put_in_functions_b():
    try:
        ...
        FUNCTION_HEADER = 'def %s(yearly_salary, portion_saved, cost_of_dream_home, semi_annual_raise):' % (
            FUNCTION_NAME)
