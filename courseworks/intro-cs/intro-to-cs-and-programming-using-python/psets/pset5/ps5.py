...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        ...
        transformed = tuple(int(value) for value in transformed)
        filtered_pixels.append(transformed)
...
def extract_end_bits(num_end_bits, pixel):
    ...
    """
    mod_value = 2 ** num_end_bits
    if type(pixel) == int:
        return pixel % mod_value
    else:
        result = []
        for value in pixel:
            result.append(value % mod_value)
        return tuple(result)
...
def reveal_bw_image(filename):
    ...
    for px in pixels:
        bit = extract_end_bits(1, px)
        hidden_pixels.append(bit * 255)

    result = pix_to_img(hidden_pixels, (width, height), 'L')
    return result
...
def reveal_color_image(filename):
    ...
    for px in pixels:
        bit = extract_end_bits(3, px)
        scaled_pixel = []
        for value in bit:
            scaled_value = int(value * 255 / 7)
            scaled_pixel.append(scaled_value)
        hidden_pixels.append(tuple(scaled_pixel))

    result = pix_to_img(hidden_pixels, (width, height), 'RGB')
    return result
...
def main():
    ...
    # draw_kerb('Unhidden hidden1.bmp', 'Sheikh Md. Sazidul Islam')
